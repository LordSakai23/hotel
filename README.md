...
Technical survey responses will help evaluate backend efficiency, programming practices, and code structure. Non-technical survey responses will gauge user satisfaction, visual appeal, accessibility, and engagement.
...
3. Problems with code security and code repetition
...
3. Encouragement related to the features of the application
4. Positive recognition of the environmental theme and purpose of the application
This feedback will be compiled and used to inform further iterations and improvements of the application.
...
Technical Focus Group Responses
1. Is the code well-commented and easy to follow?
Yes, the code is well-commented. Key functions, such as the carbon footprint calculator logic and energy usage tracking modules, include brief summaries describing their purpose. However, some of the methods and database interactions could benefit from more detailed comments. Overall, a developer familiar with C# would be able to follow the code with minimal confusion, but additional comments, especially around calculation formulas and data retrieval would improve clarity.
2. Are the variable names descriptive and appropriate?
Variable names are mostly descriptive and align with best practices. Some internal variables (e.g. loop counters like “x” or “temp1”) could be renamed to more meaningful terms to enhance readability and prevent repetition, especially in functions that handle dynamic input or iterate over energy data entries.
3. What’s the best approach to linking this code to the database?
Currently, the application uses Windows Forms.NET framework with SQL commands for data access, which works but is prone to SQL injection if not carefully handled. A better approach might be to implement Entity Framework, which would simplify data manipulation using LINQ and abstract the SQL layer. This would also make the code more maintainable and easier to adapt if the data model evolves. 
4. Are the carbon footprint calculations accurate and efficient?
The carbon footprint calculator appears to use standard formulas, factoring in parameters such as electricity usage (in kWh), gas consumption, and transport data (e.g., vehicle mileage). The calculations are accurate from a logic mathematical view and are structured to allow easy adjustments for different units or emission factors. However, efficiency could be improved by isolating repeated calculations into reusable methods. Also, including unit tests to validate different scenarios (e.g., low/high energy users, zero input) would enhance reliability and make debugging easier.

Non-Technical Focus Group Responses 
1. Is the colour scheme visually appealing and appropriate?
Yes, the colour scheme is very appropriate. The use of green shades throughout the interface instantly gives off an “eco-friendly” and energy-conscious vibe, which aligns well with the purpose of the app. The contrast between background and text is good, making it readable. The inclusion of icons helps break up the green and keeps the interface from feeling too monochrome or overwhelming.

2. Does the user interface feel intuitive and easy to use?
Overall, yes. The layout is simple and clearly designed with the user in mind, and the buttons are labelled in plain language (e.g., "Home", "Menu", "Calculate Footprint"). First-time users can figure out where to go without much trial and error. However, adding short tooltips or a brief guide could make things even smoother for users unfamiliar with the topic.

3. Is it easy to navigate through the application?
Navigation is quite smooth. The menu is persistent and properly structured, with sections like “Home,” “Menu,” “Settings,” and “Profile” all easily accessible. The transitions between screens are fast, and users can return to the homepage with a single tap or click. The back button also works as expected, which adds to the feeling of control. It’s easy to find specific features like the calculator products.
...
Survey Responses
Technical survey responses
The link to this survey is https://forms.gle/VJg8tqmWYgcpnhmb8 and the survey was sent out with the file which is the screen recording of the code itself called Technical recording.
...
Non-Technical Survey Response
The link to this survey is https://forms.gle/1SXJLxKyjG9azZTQA and this survey was sent out with a file called Non-Technical recording, with displays the UI of the application.
